...
class ObjectTypeComposer {
  ...
  merge(type) {
    ...
    if (type instanceof ObjectTypeComposer || type instanceof _InterfaceTypeComposer.InterfaceTypeComposer) {
      tc = type;
      ...
      tc = _InterfaceTypeComposer.InterfaceTypeComposer.createTemp(type, this.schemaComposer);
    } else {
      throw new Error(`Cannot merge ${(0, _misc.inspect)(type)} with ObjectType(${this.getTypeName()}). Provided type should be GraphQLInterfaceType, GraphQLObjectType, InterfaceTypeComposer or ObjectTypeComposer.`);
  ...
  _relationWithResolverToFC(opts, fieldName = '') {
    const resolver = (0, _is.isFunction)(opts.resolver) ? opts.resolver(this.schemaComposer) : opts.resolver;
